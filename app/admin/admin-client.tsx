...
export function AdminClient({ initiallyAuthenticated, initialShareId }: { initiallyAuthenticated: boolean; initialShareId?: string; initialEntryId?: string }) {
  ...
  function removeProject() {
    const project = content.projects[selectedIndex];
    if (!project || !window.confirm(`确定删除项目“${project.title}”？删除后需点击“保存并发布”才会公开生效。`)) return;
    setContent((current) => ({ ...current, projects: current.projects.filter((_, index) => index !== selectedIndex) }));
    setSelectedIndex((current) => Math.min(current, Math.max(0, content.projects.length - 2)));
    setDirty(true);
  }
...
function ProjectEditor({ project, onText, updateProject, removeProject }: { project: ManagedProject; onText: (key: string) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void; updateProject: (patch: Partial<ManagedProject>) => void; removeProject: () => void }) {
  return <><div className="admin-editor-heading"><div><p className="eyebrow">Project</p><h2>{project.title}</h2></div><label className="admin-toggle"><input type="checkbox" checked={project.isPublished} onChange={(event) => updateProject({ isPublished: event.target.checked })} />公开显示</label><button type="button" className="button danger" onClick={removeProject}>删除项目</button></div><div className="admin-fields two-columns"><Field label="标题"><input value={project.title} onChange={onText("title")} /></Field><Field label="slug"><input value={project.slug} onChange={onText("slug")} /></Field><Field label="分类"><select value={project.categoryId} onChange={(event) => updateProject({ categoryId: event.target.value as ManagedProject["categoryId"] })}>{projectCategories.map((category) => <option key={category.id} value={category.id}>{category.label}</option>)}</select></Field><Field label="GitHub 链接"><input value={project.githubUrl} onChange={onText("githubUrl")} /></Field><Field label="线上链接（可选）"><input value={project.liveUrl ?? ""} onChange={onText("liveUrl")} /></Field><Field label="封面图 URL（可选）"><input value={project.image ?? ""} onChange={onText("image")} /></Field></div><div className="admin-fields"><Field label="项目摘要"><textarea value={project.summary} onChange={onText("summary")} /></Field><Field label="详情介绍"><textarea value={project.intro} onChange={onText("intro")} /></Field><WorkflowEditor project={project} updateProject={updateProject} /><Field label="图库 URL（每行一个，可使用 /projects/...）"><textarea value={(project.galleryImages ?? []).join("\n")} onChange={(event) => updateProject({ galleryImages: event.target.value.split("\n").map((item) => item.trim()).filter(Boolean) })} /></Field><Field label="图片说明（与图库逐行对应，可留空）"><textarea value={(project.galleryCaptions ?? []).join("\n")} onChange={(event) => updateProject({ galleryCaptions: event.target.value.split("\n") })} /></Field></div></>;
}
