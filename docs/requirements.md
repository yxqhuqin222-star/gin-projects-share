...
- 联系方式：邮箱、微信、代码仓库和即刻。
- 咨询入口：右下角悬浮按钮、站内对话窗口、消息接口、飞书转接适配层；没有飞书凭证时使用本地 mock 回复完成链路验证；本地真实发送优先沿用“小明”飞书机器人的私聊会话。
...
- 咨询入口在桌面和移动端都能打开、输入、发送并显示回复；无飞书凭证时 mock 链路可完整走通；本机具备 `lark-cli` 和小明私聊会话配置时，能把咨询消息发到飞书私聊；有飞书 Open API 测试凭证时能向指定飞书测试会话发送消息。
