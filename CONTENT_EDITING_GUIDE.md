...
配置 `workflow` 后，详情只显示标题、`intro`、流程、功能点和操作入口，原有 `paragraphs` 保留但不重复展示；未配置的项目继续使用原模板。没有封面图时，首页用同一流程组件展示功能。后台只编辑流程模板字段，正文段落保留用于旧模板兼容，不在后台维护。线上 D1 已保存的内容优先于源码默认值，修改默认值不会覆盖后台内容。
